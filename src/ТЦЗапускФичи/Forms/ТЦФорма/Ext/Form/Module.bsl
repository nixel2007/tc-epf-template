...
&НаКлиенте
Функция ТЦУникальныйНомерВРМ(ПредварительныйФильтрПоИмениКомпьютера = Ложь)
	
	Роль = ТЦИмяРоли();
	НомерВРМ = ТЦНомерВРМ();
	ИмяКомпьютера = ТЦИмяКомпьютера();
	
	Возврат ТЦУникальныйНомерВРМнаСервере(Роль, НомерВРМ, ИмяКомпьютера, ПредварительныйФильтрПоИмениКомпьютера);
	
КонецФункции

&НаСервере
Функция ТЦУникальныйНомерВРМнаСервере(
	Роль,
	НомерВРМ,
	ИмяКомпьютера,
	ПредварительныйФильтрПоИмениКомпьютера = Ложь)
...
Процедура ТЦПауза(РазмерПаузы)
	...
	КипВК = ТЦПолучитьОбщийМодульПоИмени("КипВнешнийКомпонент");
	Если КипВК = Неопределено Тогда
		ВызватьИсключение "Процедура ТЦПауза не поддерживается в изолированном режиме";
	КонецЕсли;
	
	Инструменты = КипВК.ПолучитьИнструменты();	
	КипВК.Пауза(Инструменты, РазмерПаузы * 1000);
...
Процедура ЗаполнитьНастройкиVA()
	...
	НачальныйПорт = 48000;
	СмещениеПорта = ТЦУникальныйНомерВРМ(Истина);
	...
	Если Не ПустаяСтрока(ТЦОбъект.ДополнительныеПараметрыКлиентовТестирования) Тогда
		ПрименитьДополнительныеПараметрыКлиентовТестирования(ФормаVA);
	КонецЕсли;
	
	Если ТЦОбъект.КаталогиБиблиотек.Количество() > 0 Тогда
		ПрименитьКаталогиБиблиотек(ФормаVA);
	КонецЕсли;	
		
КонецПроцедуры

&НаКлиенте
Процедура ПрименитьДополнительныеПараметрыКлиентовТестирования(ФормаVA)
	
	Для Каждого ДанныеКлиента Из ФормаVA.ДанныеКлиентовТестирования Цикл
		
		ЭтоНовыеПараметры = СтрНайти(ДанныеКлиента.ДопПараметры, ТЦОбъект.ДополнительныеПараметрыКлиентовТестирования) = 0;
		
		Если ЭтоНовыеПараметры Тогда
						
			Если ПустаяСтрока(ДанныеКлиента.ДопПараметры) Тогда
				ДанныеКлиента.ДопПараметры = ТЦОбъект.ДополнительныеПараметрыКлиентовТестирования;
			Иначе
				ДанныеКлиента.ДопПараметры = СтрШаблон("%1 %2", ДанныеКлиента.ДопПараметры, ТЦОбъект.ДополнительныеПараметрыКлиентовТестирования);
			КонецЕсли;
...
&НаКлиенте
Процедура ПрименитьКаталогиБиблиотек(ФормаVA)
	
	МассивКаталогов = ТЦОбъект.КаталогиБиблиотек.ВыгрузитьЗначения();
	
	ФормаVA.Объект.КаталогиБиблиотек.ЗагрузитьЗначения(МассивКаталогов);
...
&НаКлиенте
Процедура ОстановитьВсеЗамеры()
	// Факт наличия замера хранится в глобальной переменной.
	// Конкретное место значения меняется на разных БСП.
	//
	// Общий алгоритм:
	// Если СуществуетПеременнаяЗамеров Тогда
	//	ОбнулитьПеременнуюЗамеров();
	// КонецЕсли;
	
	// БСП 2.2
	Если Вычислить("ОценкаПроизводительностиЗамерВремени") <> Неопределено Тогда
		// Сбросим сохраненные данные по всем замерам, не записанным явно.
		ОценкаПроизводительностиЗамерВремени = Неопределено;
	КонецЕсли;
	
	// БСП 2.3+
	Если Вычислить("ПараметрыПриложения") <> Неопределено Тогда
		ПараметрыПриложения["СтандартныеПодсистемы.ОценкаПроизводительностиЗамерВремени"] = Неопределено;
	КонецЕсли;
