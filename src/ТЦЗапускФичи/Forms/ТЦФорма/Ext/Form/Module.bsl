...
&НаСервере
Функция ТЦРеальныйНомерВРМ(Роль, НомерВРМ, ИмяКомпьютера, ПредварительныйФильтрПоИмениКомпьютера = Ложь)
...
Процедура ЗаполнитьНастройкиVA()
	...
	НачальныйПорт = 48000;
	СмещениеПорта = ТЦРеальныйНомерВРМ(
		ТЦРоль(), 
		ТЦНомерВРМ(), 
		ТЦИмяКомпьютера(), 
		Истина
...
Процедура ПеренестиНастройкиРолиВКонтекстVA()
	...
	ФормаVA.ОбъектКонтекстСохраняемый.Вставить("КлючеваяОперация", ТЦОбъект.КлючеваяОперация);	
